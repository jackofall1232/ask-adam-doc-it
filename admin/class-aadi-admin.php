...
class AADI_Admin {
	...
	public function render_size_limit_notice() {
		...
		if ( class_exists( 'AADI_Search' ) && AADI_Search::is_over_limit() ) {
			printf(
				'<div class="notice notice-warning"><p>%s</p></div>',
				esc_html(
					sprintf(
						/* translators: 1: document count threshold, 2: number of documents scored per AI search. */
						__( 'Ask Adam Doc It: Your library exceeds %1$d documents. The free version scores only the most recent %2$d during AI search. Upgrade to Pro for full-library vector search.', 'ask-adam-doc-it' ),
						AADI_Search::MAX_SCORED_POSTS,
						AADI_Search::MAX_SCORED_POSTS
					)
				)
			);
